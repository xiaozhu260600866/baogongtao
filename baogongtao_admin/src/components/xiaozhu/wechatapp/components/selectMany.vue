<template>
  <section>
    <view v-if="thisDiag" class="share-overlay" @click="toggleDiag" />
    <div v-if="thisDiag" style="z-index:9999">
      <div class="class_left">
        <div v-for="(parent,key) in data" :class="['li',actIndex == key ? 'cur' :'']" @click="actIndex = key"><span class="label">{{ parent.label }}</span></div>
      </div>
      <div class="class_right">
        <div class="index-tb rlist">
          <div v-for="(son,key1) in data[actIndex].children" class="ta-tr" @click="choose(son)"><a class="tb-item">{{ son.label }}</a></div>
        </div>
      </div>
    </div>
  </section>
