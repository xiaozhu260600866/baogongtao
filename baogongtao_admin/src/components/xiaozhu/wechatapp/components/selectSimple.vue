...
<template>
  <section>
    <div class="search-select">
      <picker :value="actIndex" class="order-picker fs12 font_grey" :range="data" range-key="label" @change="selectRes">
        {{ data[actIndex].label }}</picker>

      <i class="iconfont icon-bottom fs16 font_grey lh30" style="position:absolute;right:5px" />
    </div>
  </section>
</template>
