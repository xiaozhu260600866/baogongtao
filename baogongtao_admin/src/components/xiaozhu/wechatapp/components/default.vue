...
<template>
  <section>
    <slot name="content" />
    <myFooter v-if="footerShow" :url="formAction" />
    <userFooter v-if="footerShow2" :url="formAction" />
    <div v-if="data.show" class="float-group" :style="Fbottom">
      <slot name="floatBtn" />
...
export default {
    ...
    onUnload() {
        ...
        if (pages.length >= 2) {
            ...
            const reload = wx.setStorageSync('reload')

            const historyUrl = this.$store ? this.$store.state.mutations.historyUrl : ''
            if (historyUrl) {
