<template>
  <view class="w-picker-view">
    <picker-view class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange">
      <picker-view-column>
        <view v-for="(item,index) in range.dates" :key="index" class="w-picker-item">{{ item.label }}</view>
      </picker-view-column>
      <picker-view-column>
        <view v-for="(item,index) in range.hours" :key="index" class="w-picker-item">{{ item.label }}时</view>
      </picker-view-column>
      <picker-view-column>
        <view v-for="(item,index) in range.minutes" :key="index" class="w-picker-item">{{ item.label }}分</view>
      </picker-view-column>
    </picker-view>
  </view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			checkValue(value) {
				const strReg = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}(:\d{2})?$/; const example = '2019-12-12 18:05:00或者2019-12-12 18:05'
			...
			resetData(year, month, day) {
				...
				const months = []; const days = []; const sections = []
			...
			getData(dVal) {
				...
				const curFlag = this.current
				const disabledAfter = this.disabledAfter
				const dates = []; const hours = []; const minutes = []
			...
			getDval() {
				...
				const date = this.formatNum(year) + '-' + this.formatNum(month) + '-' + this.formatNum(day)
			...
			initData() {
				...
				let dates = []; let hours = []; let minutes = []
				let dVal = []; let pickVal = []
				const value = this.value
				const reg = /-/g
				let range = {}
				let result = ''; let full = ''; let date; let hour; let minute; let obj = {}
