  ...
  <view v-if="lotusAddressData.visible" :status="checkStatus" class="lotus-address-mask">
    <view :class="lotusAddressData.visible?'lotus-address-box':'lotus-address-box lotus-address-box-out'">
      <view class="lotus-address-action">
        <text class="lotus-address-action-cancel" @tap="cancelPicker">取消</text>
        <text class="lotus-address-action-affirm" @tap="chosedVal">确认</text>
      </view>
      <view class="lotus-address-picker-box">
        ...
        <scroll-view scroll-y :scroll-into-view="'pid'+pChoseIndex" class="lotus-address-picker-box-item">
          <view v-for="(pItem,pIndex) in province" :id="'pid'+pIndex" :key="pIndex" :class="pIndex === pChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" @tap="clickPicker(0,pIndex,pItem);">{{ pItem }}</view>
        </scroll-view>
        <!--市-->
        <scroll-view scroll-y :scroll-into-view="'cid'+cChoseIndex" class="lotus-address-picker-box-item">
          <view v-for="(cItem,cIndex) in city" :id="'cid'+cIndex" :key="cIndex" :class="cIndex === cChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" @tap="clickPicker(1,cIndex,cItem);">{{ cItem }}</view>
        </scroll-view>
        <!--区-->
        <scroll-view scroll-y :scroll-into-view="'tid'+tChoseIndex" class="lotus-address-picker-box-item">
          <view v-for="(tItem,tIndex) in town" :id="'tid'+tIndex" :key="tIndex" :class="tIndex === tChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'" @tap="clickPicker(2,tIndex,tItem);">{{ tItem }}</view>
        </scroll-view>
        <!--区END-->
      </view>
    </view>
