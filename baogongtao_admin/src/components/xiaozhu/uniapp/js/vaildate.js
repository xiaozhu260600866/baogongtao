...
vaildform.install = (Vue, options) => {
                ...
                for (var r in ruleform) {
                    if (v == r) {
                        ...
                        for (var i = 0; i < vaildArr.length; i++) {
                            const isChinaese = new RegExp('[\\u4E00-\\u9FFF]+', 'g')
                            if (vaildArr[i] == 'require' || (isChinaese.test(value) && value.indexOf('请选择') != -1)) {
								...
								const max = parseInt(vaildArr[i].split(':')[1])
								if (value.length > max) {
								...
								const min = parseInt(vaildArr[i].split(':')[1])
								if (value.length < min) {
    ...
    Vue.prototype.uploadImageForWechat = (event, that, fun) => {
        ...
        const uploadUrl = that.getSiteName + '/ajax/app-upload-pic'
    ...
    Vue.prototype.updateHeadImg = (fun) => {
       const uploadUrl = Vue.prototype.getSiteName() + '/ajax/app-upload-pic'
        wx.chooseImage({
            count: 1, // 默认9
            sizeType: ['original', 'compressed'], // 可以指定是原图还是压缩图，默认二者都有
            sourceType: ['album', 'camera'], // 可以指定来源是相册还是相机，默认二者都有
            success: res => {
                ...
                wx.uploadFile({ // 上传服务器
                    ...
                    success: res => {
                        ...
                        const file = data.filename
                        wx.hideLoading()
                        const newFile = Vue.prototype.getSiteName() + '/upload/images/user/' + file
                        fun(newFile)
                    },
                    fail: function(res) {
                        console.log(res)
