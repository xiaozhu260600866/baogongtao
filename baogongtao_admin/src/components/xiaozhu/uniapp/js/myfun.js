...
myfun.install = (Vue, options) => {
	...
	Vue.prototype.updateUrl=(urlArr)=>{
		for (let i = 0; i < urlArr.length; i++) {
			let url = urlArr[i];
			let updateUrl = uni.getStorageSync("updateUrl")  || [];
			let canPush = true;
	...
	Vue.prototype.payUniapp = (res, callBack,callBack2) => {
			...
			return new Promise((resolve, reject) => {
				if (uni.getStorageSync("source") == 'wap') {
					let wx = require('xiaozhu/weixin-js-sdk');
					wx.chooseWXPay({
	...
	Vue.prototype.goto = (url, type,refreshUrl) => {
		...
		Vue.prototype.shareSource = (that, title, notShareSource) => {
			...
			let urlPre = currentPage.route;
			let scope = "";
			for (let i in that.data.query) {
	...
	Vue.prototype.getQQvideoLink = (videoSrc, callBack) => {
		...
		let url = "https://vv.video.qq.com/getinfo?vids=" + videoSrc + "&platform=101001&charge=0&otype=json";
		return new Promise((resolve, reject) => {
			wx.request({
				url: url,
				data: {},
