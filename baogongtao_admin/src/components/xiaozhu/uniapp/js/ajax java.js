	...
	Vue.prototype.getAjax = (that, data, notLoading) => {
		...
		return new Promise((resolve, reject) => {
			...
			that.formAction.forEach((formActionData, formActionDataIndex) => {
				const canForEach = !(that.data.nextPage > 1 && formActionData.mapperValues !=
					'lists')
				if (canForEach) {
						...
						success: (msg) => {
							...
							if (formActionData.mapperKeys) {
								formActionData.mapperKeys.split(',').forEach((
									mapperKey, index) => {
									const value = formActionData.mapperValues
										.split(',')[index]
									...
									if (value == 'lists' &&
										...
										const lastPage = msg.data[mapperKey]
											.last_page
										const nextPage = parseInt(msg.data[
