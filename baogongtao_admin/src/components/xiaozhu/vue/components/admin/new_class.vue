...
<template id="class">
  <el-dialog :title="'新建|编辑'+title" :visible.sync="dialogVisible" width="1000px" :top="top" :before-close="handleClose" append-to-body>
    <div v-if="auth==1" class="flex-right mb20"><button class="dx-btn dx-btn-blue" @click="$refs.createEditClass.ajax({id:0,name:'一级'},formAction,'add')">新建{{ title }}</button></div>
    <div>
      <treeLi :data="data.lists" lev="0" :show="true" :form-action="formAction" :auth="auth" />
    </div>
...
export default {
    ...
    props: ['formAction', 'top', 'title', 'auth'],
    data: function() {
        return {
            ruleForm: {},
