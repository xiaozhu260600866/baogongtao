...
<template>
    <el-dialog title="查看记录" :visible.sync="dialogFormVisible" :top="top" :before-close="handleClose">
        <el-col :span="24" class="toolbar">
            <el-form :inline="true" :model="searchFields" style="text-align: -webkit-right;">
                <el-form-item v-for="(v,i) in searchFields">
                    <el-input :placeholder="'请输入要搜索的关键字'" name="name" v-model="searchFields[i]" v-if="i!='searchDate'"></el-input>
                </el-form-item>
                <el-date-picker type="daterange" placeholder="选择日期范围" v-model="searchDate" v-if="searchFields.searchDate"> </el-date-picker>
                <el-form-item>
                    <el-button type="primary" @click="toSearch">查询</el-button>
                </el-form-item>
            </el-form>
        </el-col>
        <el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" v-loading="listLoading">
            <el-table-column :prop="v.prop" :label="v.label" :min-width="v.minWidth" :width='v.width' v-for="v in fieldArr"> </el-table-column>
        ...
        <el-col :span="24" class="toolbar" style='margin:50px 0'>
            <el-pagination @current-change="handleCurrentChange" :current-page="data.current_page" :page-sizes="[10]" :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="data.total" style='float: right'>
            </el-pagination>
        </el-col>
    </el-dialog>
</template>
