<template>
  <div>
      ...
      <el-form :inline="true" :model="data.query" class="flex">
        <div class="flex1">
          <el-form-item v-for="v in searchFields" :label="v.label">
            <el-input v-if="!v.type || v.type == 'text'" v-model="data.query[v.prop]" :placeholder="v.placeholder ? v.placeholder : '请输入'+v.label" />
            <el-select v-if="v.type == 'select'" v-model="data.query[v.prop]" placeholder="请选择" filterable>
              <el-option v-for="q in data[v.datakey]" v-if="v.datakey" :label="q.label" :value="q.value" />
              <el-option v-for="q in v.data" v-if="v.data" :label="q.label" :value="q.value" />
            </el-select>
            <div v-if="v.type == 'betweenDate'">
              <el-date-picker v-model="data.query[v.prop+'_start'] " type="date" :placeholder="v.placeholder ? v.placeholder :'选择'+v.label+'开始范围' " value-format="yyyy-MM-dd" />~
              <el-date-picker v-model="data.query[v.prop+'_end'] " type="date" :placeholder="v.placeholder ? v.placeholder :'选择'+v.label+'结束范围'" value-format="yyyy-MM-dd" />
            </div>
            ...
            <el-button-group v-if="v.type == 'searchYear'">
              <el-button v-for="year in v.children" :type="data.query[v.prop] == year.prop ? 'primary' : '' " @click="data.query[v.prop] = year.prop;search();">{{ year.label }}</el-button>
            </el-button-group>
          </el-form-item>
          <el-form-item>
            <el-button v-if="searchFields" type="primary" @click="search">查询</el-button>
            <el-button v-if="classAction" type="primary" @click="$refs.category.ajax()">类别管理</el-button>
            <el-button v-if="exportUrl" type="success" @click="exportExcel">导出</el-button>
            <el-button v-if="uploadData && uploadData.url" type="success" @click="upload">上传</el-button>
          </el-form-item>
        ...
        <div class="Hright">
          ...
          <el-form-item class="mr0">
            <el-button v-if="canCreate" type="primary" @click.native="$refs.createEdit.ajax('',data,fields)">新建{{ tableName =="users" ? '会员' :'' }}</el-button>
          </el-form-item>
        </div>
      </el-form>
    </el-col>
    <slot name="header2" />
    <el-col v-if="tarbars" :span="24">
      <el-tabs v-model="data.query[tarbars.prop]" type="card" @tab-click="tabClick">
        <el-tab-pane v-for="v in tarbars.data" :label="v.count ? v.label+ '('+data[v.datakey]+')':v.label" :name="''+v.value" />
      </el-tabs>
    </el-col>
    <el-table
      ref="multipleTable"
    ...
    >
      <el-table-column v-if="checkAll" type="selection" width="50" />
      <div v-for="(v,key) in tableFields" v-show="v['hidden_'+v.prop]? canShow(v):true">
        <el-table-column v-if="!v.hidden" :prop="v.prop" :label="v.label" :width="v.width" :align="v.align ? v.align:'left'" :min-width="v.minWidth">
          <template v-if="v.type || !v.type && v['append_table_'+v.prop]" scope="scope">
            <div v-if="v.type && v.type == 'editField'">
              <el-switch v-model="scope.row[v.prop]" on-text="" off-text="" :active-value="1" :inactive-value="0" @change="editField(scope.row,v.prop,v.url)" />
            </div>
            <div v-if="v.type && v.type == 'sort'">
              <el-input v-model="scope.row[v.prop]" @change="editField(scope.row,v.prop,v.url)" />
            </div>
            <slot v-if="!v.type && v['append_table_'+v.prop]" :name="'append_table_'+v.prop" :row="scope.row" :$index="scope.$index" />
          </template>
        </el-table-column>
      </div>
      <el-table-column label="操作" :width="operateWidth ? operateWidth : '160'">
        <template scope="scope">
          <div v-if="tableOperateButton">
            <el-dropdown>
              <el-button type="primary" size="mini">操作<i class="el-icon-arrow-down el-icon--right" /></el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="canEdit" @click.native="$refs.createEdit.ajax(scope.row,data,fields)">编辑</el-dropdown-item>
                <el-dropdown-item v-if="showInfo" @click.native="refs.createEdit.ajax(scope.row,data,fields,'info')">查看</el-dropdown-item>
                <el-dropdown-item v-if="scope.row.status!=1 && auditAction" @click.native="changeStatus(scope.$index, scope.row,'1')">审核通过</el-dropdown-item>
                <el-dropdown-item v-if="scope.row.status!=2 && auditAction" @click.native="changeStatus(scope.$index, scope.row,'2')">审核拒绝</el-dropdown-item>
                <slot name="operate" :row="scope.row" />
                <el-dropdown-item v-if="canDel" @click.native="handleDel(scope.$index,scope.row)">删除</el-dropdown-item>

              </el-dropdown-menu>
            </el-dropdown>
          </div>
          <div v-else class="cell_group">
            <el-button v-if="canEdit" type="primary" size="mini" @click.native="$refs.createEdit.ajax(scope.row,data,fields)">编辑</el-button>
            <el-button v-if="showInfo" type="primary" size="mini" @click.native="$refs.createEdit.ajax(scope.row,data,fields,'info')">查看</el-button>
            <el-button v-if="scope.row.status==0 && auditAction" type="primary" size="mini" @click="changeStatus(scope.$index, scope.row,'1')">审核通过</el-button>
            <el-button v-if="scope.row.status== 0&& auditAction" type="primary" size="mini" @click="changeStatus(scope.$index, scope.row,'2')">审核拒绝</el-button>
            <div class="slot_btn"><slot name="operate" :row="scope.row" /></div>
            <el-button v-if="canDel" class="canDel" size="mini" @click.native="handleDel(scope.$index,scope.row)">删除</el-button>
          </div>
    ...
    <div class="toolbar foot-tool">
      <el-button v-if="checkAll" type="danger" :disabled="this.sels.length===0 " @click="delAll">批量删除</el-button>
      <el-button v-if="putaway" type="danger" :disabled="this.sels.length===0 " @click="puawayAll(1)">批量上架</el-button>
      <el-button v-if="putaway" type="danger" :disabled="this.sels.length===0 " @click="puawayAll(0)">批量下架</el-button>
      <slot name="tableLeft" :row="sels" />
      <el-pagination
        :current-page="data.thisPage"
        :page-sizes="[pageSize]"
        :page-size="pageSize"
    ...
    <my-create-edit ref="createEdit" :fields="formFields" :data="data" :create-action="createAction" :edit-action="editAction" :create-edit-label="createEditLabel" :create-edit-diag-width="createEditDiagWidth" @createBeforeCallBack="createBeforeCallBack" @submitBeforeCallBack="submitBeforeCallBack">
    ...
    <uploadFile v-if="uploadData && uploadData.url" ref="upload" :action="uploadUrl()" :down-load-url="uploadData.downloadUrl" />
    <my-class v-if="classAction" ref="category" top="2%" :online="online" :type="classType" :form-action="classAction" :sizearr="300" :canupload="canupload" :page-url="classUrl" />
  </div>
</template>
<script>
	export default {
		...
		methods: {
			...
			handleDel(index, item) {
				const delAction = this.delAction ? this.delAction : ''
