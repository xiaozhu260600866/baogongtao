...
<template>
    ...
    <el-dialog :title="ruleForm.status==2 ? '请填写拒绝内容' :'确认是否通过' " :visible.sync="dialogVisible" size="tiny" :before-close="handleClose"  append-to-body>
        <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm" >
            <div class="el-form-item" v-if="ruleForm.status==2">
                <el-form-item label="拒绝理由" prop="content" :rules="[{ required: true, message: '请填写拒绝理由'}, ]">
                    <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="ruleForm.content"> </el-input>
                </el-form-item>
            </div>
        </el-form>
...
export default {
    ...
    props: [ /*'status','id',*/ 'formAction','source'], //父类的数据
    methods: {
