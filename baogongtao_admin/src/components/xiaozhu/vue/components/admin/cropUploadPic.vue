<template>
  <section class="bgf">
    <el-dialog title="选择图片" :visible.sync="dialogFormVisible" top="141px" :before-close="handleClose" width="90%">
      <el-form ref="ruleForm" :model="ruleForm" lclass="demo-ruleForm">
        <div class="flex">
          <div id="cut_container" style=" background-color: rgba(255, 255, 255, 0.3); width: 600px; height: 800px;margin-right: 30px">
            <div style="width: 300px;  background-color: rgba(255, 255, 255, 0.9);">
              <input id="change_btn" type="file" style="width: 80%; margin-top: 5%;display: block" @change="cut.fileChange('change_btn')">
              按回车裁剪
            </div>
          ...
          <div style="width:600px;height:600px">
            <h3>裁剪预览框</h3>
            <div id="preview_btn" class="btn btn-primary block col-sm-12 hidden" style="margin-top: 5%;" @click="cut.previewCutImg()">预览剪裁图片</div>
            <div class="pre-container flex-middle flex-center"><img id="abc"></div>
     ...
     upload2() {
        ...
        t.toBlob(blob => {
            ...
            onSuccess: res => {
              ...
              this.currentValue = { name: res.filename,
                  url: this.getSiteName() + '/upload/images/' + this.data.data.upurl + '/' + res.filename }
            },
...
class Cut {
    constructor(img_src) {
        ...
        this.container.cut.ondblclick = event => this.ondblclick(event)
    ...
    fileChange(id) {
        ...
        reader.onload = function(e) {
            ...
            // container.cutCtx.rect(100, 300 , 300, 200);
    ...
    downloadCutImg() {
        ...
        t.height = imgData.height
        t.getContext('2d').putImageData(imgData, 0, 0)
        const src = t.toDataURL('image/octet-stream')
        const old = document.getElementById('download_img')
        if (old) {
            old.remove()
        }
...
/*        save.addEventListener('click', function () { let win = window.open();
    ...
    onMouseDown(e) {
        ...
        if (pro.readyResize === true) {
            ...
            // 准备移动
            const dot = this.getMouseCoord(e)
            pro.moveX = dot.x
            pro.moveY = dot.y
            pro.startMove = true
            pro.readyMove = false
    ...
    onMouseMove(e) {
        ...
        if (pro.isCutting === true || pro.isResizing === true) {
            ...
            // 移动图像
            const dot = this.getMouseCoord(e)
                const offsetX = Math.abs(dot.x - pro.moveX); const // 移动x距离
                offsetY = Math.abs(dot.y - pro.moveY)
                let x; let y; let ex; let ey// 移动y距离
    ...
    drawCute(x, y, ex, ey) {
        const pro = this.property
            const container = this.container
        const r = pro.cuteLength; const // 方块为2个像素
            color = 'green'// 颜色
        container.cutCtx.save()
        container.cutCtx.fillStyle = color
        const tx = x || pro.startX
            const ty = y || pro.startY
            const tex = ex || pro.validEX
            const tey = ey || pro.validEY
    ...
    drawSize(x1, y1, ex, ey) {
        ...
        const fillStyle = 'rgba(0,0,0,0.7)'
            ...
            const tex = ex || pro.validEX
            const tey = ey || pro.validEY
            const offsetX = x - pro.validEX
            const offsetY = y - pro.validEY
        ...
        const width = parseInt(Math.abs(pro.realStartX - pro.realEndX))
            const height = parseInt(Math.abs(pro.realStartY - pro.realEndY))
        if (offsetX <= 0) {
            if (offsetY <= 0) {
                // 以startX,startY宽高点
            } else {
    ...
    // 获取鼠标在外层canvas的坐标
    getMouseCoord(e) {
        const site = this.container.cut.getBoundingClientRect()
        return {
...
class CutContainer {
    ...
    createCut() {
        const styles = {
            ...
            let h = document.getElementById('cut_container').style.height
            var w = document.getElementById('cut_container').style.width
    ...
    createLvCanvas(img) {
        ...
        const width = this.cut.width - 40
        const height = this.cut.height - 40
