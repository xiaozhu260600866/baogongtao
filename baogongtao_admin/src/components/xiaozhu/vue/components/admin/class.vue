...
2.      <my-class top="2%" ref="category" :type="type" formAction="/admin/article/"></my-class>   //注意formAction 是方法 的前缀
 -->
<template id="class">
  <el-dialog title="新建\编辑类别" :visible.sync="dialogVisible" width="1000px" :top="top" :before-close="handleClose" :close-on-click-modal="false">
    <div>
      <table v-loading="listLoading" class="table table-hover mt10">
        <thead>
          <tr class="ta_th_tr">
            <th width="50">排序</th>
            <th width="220">名称</th>
            <th width="120">显示/隐藏</th>
            <th v-if="pageUrl" width="200">url链接</th>
            <th width="200">操作</th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr :id="'tr_'+v.id" class="active">
              <td align="center">
                <input
                  ...
                  type="text"
                  name="sort"
                  class="form-control input_w50 text_center v_name noborder"
              ...
              <td>
                <div class="flex">
                  ...
                  <input
                    ...
                    type="text"
                    name="sort"
                    class="form-control input_w200 v_name noborder"
              ...
              <td class="flex-center">
                <a v-if="canupload" href="javascript:;" class="btn btn-sm btn-default mr5" @click="uploadPic(v.id,v.cover)">{{ v.cover ? '已上传图片' :'上传图片' }}</a>
                <div v-if="getSiteName == 'https://renda.doxinsoft.com/'">
                  <a
                    v-if="!in_array(v.id,[97,91,88,87,86])"
                    href="javascript:;"
                    class="fs12 btn btn-default del_fc"
                    :data-id="v.id"
                    :data-url="formAction"
            ...
            <template v-if="v.zclass">
              <template v-for="(son,key2) in v.zclass">
                <tr :id="'tr_'+son.id" :class="'zclass_'+v.id" :style="fid ? fid==v.id && show ? '' : 'display:none' : 'display:none' ">
                  ...
                  <td align="center">
                    <a v-if="canupload" href="javascript:;" class="btn btn-sm btn-default" @click="uploadPic(son.id,son.cover)">{{ son.cover ? '已上传图片' :'上传图片' }}</a>
                    <a
                      href="javascript:;"
                      class="btn btn-sm btn-default del_zc"
                      :data-id="son.id"
                      :data-url="formAction"
                      @click="delzc(formAction,son.id,key,key2)"
                    >删除子类别</a>
                  </td>
                </tr>
              </template>
              ...
              <tr :id="'add_btn_'+v.id" :class="' zclass_'+v.id" :style="fid ? fid==v.id && show ? '' : 'display:none' : 'display:none' ">
                <td />
                <td colspan="4" class="ope01 seclist pl50"><a
                  href="javascript:;"
                  class="btn btn-sm"
                  :data-id="v.id"
                  @click="addZc(v.id)"
              ...
              <tr :id="'add_form_'+v.id" style="display:none">
                <td align="center">
                  <input v-model="sort" type="text" name="sort" class="form-control input_w50 text_center">
                </td>
                <td :colspan="pageUrl ? 3 : 2" class="ope01 seclist pl50">
                  <input
                    v-model="name"
                    type="text"
                    name="name"
                    class="form-control input_w300"
                    datatype="*"
                  >
                </td>
                <td align="center">
                  ...
                  <input
                    class="btn btn-sm btn-default plr20"
                    type="button"
                    :data-id="v.id"
                    value="取消"
          ...
          <tr id="add_btn_">
            <td />
            <td colspan="4" class="ope01 seclist"><a
              href="javascript:;"
              class="btn btn-sm"
              data-id=""
              @click="addZc('')"
            ><span class="glyphicon glyphicon-plus-sign" /> 新增父类别</a>
            </td>
          </tr>
          <tr id="add_form_" style="display:none">
            ...
            <td class="ope01 seclist" :colspan="pageUrl ? 3 : 2">
    ...
    <el-dialog title="提示" :visible.sync="uploadVisible" size="tiny" :before-close="uploadClose" append-to-body>
      <div class="el-form-item">
        <my-upload upurl="product" :upload-length="1" :file-list="fileList" :sizearr="[120]" opentype="ablank" label="上传图片" message="最佳尺寸：640*640" />
      </div>
...
<script type="text/javascript">
    export default {
        ...
        props: ['formAction', 'canupload', 'type', 'top', 'pageUrl', 'online'],
        data: function() {
            return {
                ruleForm: {},
        ...
        methods: {
            ...
            ajax: function() {
                ...
                this.formAction = this.formAction + 'class?type=' + this.type + '&token=' + Math.random() * 10 + '&online=' + this.online
