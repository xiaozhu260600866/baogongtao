<template>
  <div>
      ...
      <el-dialog title="类别" :visible.sync="dialogVisible" :width="(width +60) + 'px'" :before-close="handleClose">
        <div class="dx-header">
          <div style="padding: 4px 40px 4px 0;">
            <el-input v-model="searchValue" placeholder="搜索名称" prefix-icon="el-icon-search" />
            <div
              v-permission="['client','hr.department.operate']"
              class="pointer"
              style="position: absolute;right: 25px;top: 30px;font-size: 26px;color: #666;font-weight: bold;"
              @click="add"
            ><i class="el-icon-plus" style="font-size: 26px;" />
            </div>
          </div>
        </div>
        <div
          class="el-table el-table--fit el-table--enable-row-hover el-table--enable-row-transition el-table--medium dx-xiaozhu"
          style="width: 100%;"
        >
          <div class="el-table__header-wrapper">
            <table cellspacing="0" cellpadding="0" border="0" class="el-table__header">
              <colgroup>
                <col v-for="v in fields" v-if="!v.tableHidden" name="el-table_2_column_5" :width="v.width">
                <col name="el-table_2_column_5" width="300">
              </colgroup>
              <thead class="has-gutter">
                <tr class="">
                  <th v-for="v in fields" v-if="!v.tableHidden" colspan="1" rowspan="1" class="el-table_2_column_6 is-leaf">
    ...
    <div v-else>
      <div class="dx-header">
        <div style="padding: 4px 40px 4px 0;">
          <el-input placeholder="搜索名称" prefix-icon="el-icon-search" />
          <div
            v-permission="['client','hr.department.operate']"
            class="pointer"
