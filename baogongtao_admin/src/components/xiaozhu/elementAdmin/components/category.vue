      ...
      <tbody>
        <template v-for="(v,key) in lists" v-if="v.type==type">
          <tr :id="'tr_'+v.id" class="active">
            <td class="sort">
              <input :id="'sort_'+v.id" v-model="v.sort" type="text" name="sort" class="table-input" @blur="blurField(v.id,v.sort,'sort')">
            </td>
            <td class="flex">
            ...
            <td v-if="url">
              <input v-model="v.url" type="text" name="url" class="table-input" @blur="blurField(v.id,v.url,'url')">
            </td>
            <td align="center">
          ...
          <template v-if="v.zclass">
            <template v-for="(son,key2) in v.zclass">
              <tr :id="'tr_'+son.id" :class="'zclass_'+v.id" :style="fid ? fid==v.id && show ? '' : 'display:none' : 'display:none' ">
                <td class="sort">
                  <input v-model="son.sort" type="text" name="sort" class="table-input" @blur="blurField(son.id,son.sort,'sort')"></td>
                <td class="ope01 seclist pl50">
                  <input v-model="son.name" type="text" name="sort" class="table-input" @blur="blurField(son.id,son.name,'name')">
                </td>
                <td v-if="url" class="ope01 seclist pl50">
                  <input v-model="son.url" type="text" name="url" class="table-input" @blur="blurField(son.id,son.url,'url')">
                </td>
                <td>
            ...
            <tr :id="type+'add_form_'+v.id" style="display:none">
              ...
              <td v-if="url" class="name pl50">
                <input v-model="url" type="text" name="url" class="form-control input_w300" datatype="*">
              </td>
              <td align="center">
        ...
        <tr :id="type+'add_btn_'">
          <td />
          <td colspan="2"><div class="nav nav-primary" data-id="" @click="addZc('')">新增</div>
          </td>
        </tr>
        <tr :id="type+'add_form_'" style="display:none">
          ...
          <td v-if="url" class="ope01 seclist pl15">
            <input v-model="url_" type="text" name="url" class="form-control input_w300" datatype="*">
          </td>
          <td align="center">
...
export default {
	...
	props: ['actionUrl', 'lists', 'canupload', 'type', 'actionPrefix', 'hasSon', 'url'],
	data: function() {
		return {
			name: '',
			sort: 0,
