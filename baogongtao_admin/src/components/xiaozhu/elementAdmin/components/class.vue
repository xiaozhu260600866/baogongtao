...
2.      <my-class top="2%" ref="category" :type="type" formAction="/admin/article/"></my-class>   //注意formAction 是方法 的前缀
 -->
<template id="class">
    <el-dialog title="新建\编辑类别" :visible.sync="dialogVisible" width="1000px" :top="top" :before-close="handleClose"
        :closeOnClickModal="false">
        <div>
            <table class="table" v-loading="listLoading" cellpadding="0" cellspacing="0">
                <thead>
                    <tr class="ta_th_tr">
                        <th>排序</th>
                        <th>名称</th>
                ...
                <tbody>
                    <template v-for="(v,key) in lists">
                        <tr :id="'tr_'+v.id">
                            <td align="center" class="sort">
                                <input type="text" name="sort" class="table-input" @blur="blurField(v.id,v.sort,'sort')"
                                    :id="'sort_'+v.id" v-model="v.sort" />
                            </td>
                            <td class="name">
                                <div class="flex">
                                    <i class="ico_sh" :data-id="v.id" @click="toggleChild(v.id)"><img src="https://boss.doxinsoft.com/images/arrow_down.png"
                                            v-if="!classChildrenHidden" /></i>
                                    <input type="text" name="sort" v-model="v.name" class="table-input" @blur="blurField(v.id,v.name,'name')" />
                                </div>
                            </td>
                            <td align="center">
                                <el-switch v-model="v.can_show" on-text="" off-text="" :active-value="1"
                                    :inactive-value="0" @change="blurField(v.id,v.can_show,'can_show')"> </el-switch>
                            </td>
                            <td v-if="pageUrl">
                            ...
                            <td v-if="no" align="center">
                                <div class="flex">
                                    <i class="ico_sh" :data-id="v.id" @click="toggleChild(v.id)"></i>
                                    <input type="text" name="sort" v-model="v.no" class="table-input" @blur="blurField(v.id,v.no,'no')" />
                                </div>
                            </td>
                            <td v-if="remark">
                                <div class="flex">
                                    <i class="ico_sh" :data-id="v.id" @click="toggleChild(v.id)"></i>
                                    <input type="text" name="sort" v-model="v.remark" class="table-input" @blur="blurField(v.id,v.remark,'remark')" />
                                </div>
                            </td>
                            <td>
                                <div class="flex">
                            ...
                            <td class="flex-center">
                                <div class="nav nav-primary" v-if="canupload" @click="uploadPic(v.id,v.cover)">{{ v.cover ? '已上传图片' :'上传图片' }}</div>
                                <div v-if="getSiteName == 'https://renda.doxinsoft.com/'">
                                    <div class="nav nav-danger" :data-id="v.id" :data-url="formAction" @click="delfc(formAction,v.id,key)"
                                        v-if="!in_array(v.id,[97,91,88,87,86])">删除父类别</div>
                                </div>
                                <div v-else>
                        ...
                        <template v-if="v.zclass">
                            <template v-for="(son,key2) in v.zclass">
                                <tr :class="'zclass_'+v.id" :id="'tr_'+son.id" :style="fid ? fid==v.id && show ? '' : 'display:none' : 'display:none' ">
                                    <td align="center" class="sort">
                                        <input type="text" name="sort" class=" table-input" @blur="blurField(son.id,son.sort,'sort')"
                                            v-model="son.sort" />
                                    </td>
                                    <td class="ope01 seclist pl50">
                                        <input type="text" name="sort" class="table-input" @blur="blurField(son.id,son.name,'name')"
                                            v-model="son.name" />
                                    </td>
                                    <td align="center">
                                    ...
                                    <td class="ope01 seclista" v-if="no">
                                        <input type="text" name="no" class="table-input" @blur="blurField(son.id,son.no,'no')"
                                            v-model="son.no" />
                                    </td>
                                    <td class="ope01 seclist " v-if="remark">
                                        <input type="text" name="remark" class="table-input" @blur="blurField(son.id,son.remark,'remark')"
                                            v-model="son.remark" />
                                    </td>
                                    ...
                                    <td align="center">
                                        <div class="nav nav-primary" v-if="canupload" @click="uploadPic(son.id,son.cover)">{{ son.cover ? '已上传图片' :'上传图片' }}</div>
                                        <div class="nav nav-danger" :data-id="son.id" :data-url="formAction" @click="delzc(formAction,son.id,key,key2)">删除子类别</div>
                                    </td>
                                </tr>
                            </template>
                            <!-- 添加子类别 -->
                            <tr :class="' zclass_'+v.id" :id="type+'add_btn_'+v.id" :style="fid ? fid==v.id && show ? '' : 'display:none' : 'display:none' ">
                                <td></td>
                                <td colspan="4" class="pl50">
                                    <div class="nav nav-primary" :data-id="v.id" @click="addZc(v.id)">新增子类别</div>
                                </td>
                            </tr>
                            <tr class="addson" :id="type+'add_form_'+v.id" style="display:none">
                                <td class="sort" align="center">
                                    <input type="text" v-model="sort" name="sort" class="table-input" />
                                </td>
                                <td :colspan="pageUrl ? 3 : 2" class="name pl50">
                                    <input type="text" v-model="name" name="name" class="table-input" datatype="*" />
                                </td>
                                <td class="name" v-if="no">
                                    <input type="text" name="no" class="table-input ml15" datatype="*" v-model="no_" />
                                </td>
                                <td class="name" v-if="remark">
                                    <input type="text" name="remark" class="table-input ml15" datatype="*" v-model="remark_" />
                                </td>
                                <td align="center">
                    ...
                    <tr :id="type + 'add_btn_'">
                        ...
                        <td colspan="4" class="ope01 seclist">
                            <div @click="addZc('')" class="nav nav-primary" data-id=""><span class="glyphicon glyphicon-plus-sign"></span>
                                新增父类别</div>
                        </td>
                    </tr>
                    <tr :id="type + 'add_form_'" style="display:none">
                        <td class="sort" align="center">
                            ...
                            <input type="text" name="sort" class="table-input" v-model="sort" />
                        </td>
                        <td class="name" :colspan="pageUrl ? 3 : 2">
                            <input type="text" name="name" class="table-input ml15" datatype="*" v-model="name" />
                        </td>
                        <td class="name" v-if="no">
                            <input type="text" name="no" class="table-input ml15" datatype="*" v-model="no_" />
                        </td>
        ...
        <el-dialog title="提示" :visible.sync="uploadVisible" size="tiny" :before-close="uploadClose" append-to-body>
            <div class="el-form-item">
                <my-upload upurl="product" :uploadLength="1" :fileList="fileList" :sizearr="[120]" opentype="ablank"
                    label="上传图片" message="最佳尺寸：640*640"></my-upload>
            </div>
...
<script type="text/javascript">
    export default {
        ...
        props: ['formAction', 'canupload', 'type', 'top', 'pageUrl', 'classChildrenHidden', 'no', 'remark'], //父类的数据
        methods: {
            resetForm(formName) {
