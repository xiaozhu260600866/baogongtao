...
2.   <my-upload action="/admin/client/upload" ref="upload" downLoadUrl="/admin/example-excel?filepath=/upload/text.csv"></my-upload>
 -->
<template>
  <el-dialog title="上传" :headers="myHeaders" :visible.sync="dialoguploadVisible" :top="top" :before-close="handleClose" size="tiny">
    <el-upload class="upload-demo" drag :action="action" multiple :on-success="uploadSuccess" :data="uploadData"> <i class="el-icon-upload" />
      <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em><span style="color: red">(只能上传excel格式文件)</span></div>
      <div slot="tip" class="el-upload__tip">
        <a slot="tip" class="el-upload__tip pull-right" :href="downLoadUrl">下载示例文件</a>
      </div>
    </el-upload>
  </el-dialog>
</template>
    ...
    methods: {
        ...
        uploadSuccess(response, file, fileList) {
            ...
            if (response.status == 2) {
                this.getSuccess(response.message)
                fileList.pop()
                this.dialoguploadVisible = false
                this.$parent.ajax()
            }
