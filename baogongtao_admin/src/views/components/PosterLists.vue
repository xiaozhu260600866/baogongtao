        ...
        <el-row>
          ...
          <el-col :xs="24">
            <el-form-item label="图片">
              <tyt-upload :length="1" :file-list="formData.cover" path-name="poster" :thumbsize="[200]" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
import { lists, store, del } from '@/api/poster'
export default {
	...
	methods: {
		getData() {
			lists({ type: this.type.id }).then(res => {
		...
		handelCreate() {
			...
			this.formData = { id: '', type: this.type.id, cover: [], name: '', url: '', sort: '', def: 0, status: 1 }
		},
		handleEdit(index, row) {
			this.formData.id = row.id
			this.$set(this.formData, 'cover', row.coverUrlArr)
			this.$set(this.formData, 'name', row.name)
			this.$set(this.formData, 'sort', row.sort)
			this.$set(this.formData, 'url', row.url)
