  ...
  <section>
    <el-col :span="24" class="toolbar">
      <el-form :inline="true" :model="data.query" style="text-align: -webkit-right;">
        <el-date-picker v-model="data.query.created_at" type="daterange" placeholder="选择日期范围" />
        <el-form-item>
    ...
    <el-table ref="multipleTable" v-loading="data.listLoading" :data="data.lists" border tooltip-effect="dark" style="width: 100%">
      <el-table-column v-if="order_no" label="分销商" width="80">
        <template scope="scope">
          <div>{{ scope.row.dis ? scope.row.dis.name : '' }}</div>
        </template>
      </el-table-column>
      <el-table-column v-if="order_no" label="分销商电话" width="115">
        <template scope="scope">
    ...
    <el-col :span="24" class="toolbar" style="margin:50px 0">
      <el-pagination :current-page="data.current_page" :page-sizes="[50]" :page-size="50" layout="total, sizes, prev, pager, next, jumper" :total="data.total" style="float: right" @current-change="handleCurrentChange" />
    </el-col>
  </section>
</template>
