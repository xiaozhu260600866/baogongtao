<template>
  <section>
    <el-table ref="multipleTable" v-loading="data.listLoading" :data="data.lists" border tooltip-effect="dark" style="width: 100%">
      ...
      <el-table-column prop="getStatus" label="状态" width="60" />
      <el-table-column prop="amount" label="累计佣金" width="80" />
      <el-table-column label="下级分销商" width="100">
        <template scope="scope">一级:{{ scope.row.downCount.one }}<br><span v-if="scope.row.downCount.two">二级:{{ scope.row.downCount.two }}</span></template>
      </el-table-column>
      <el-table-column prop="clientCount" label="客户数" width="80" />
      ...
      <el-table-column label="操作" min-width="300">
        <template scope="scope">
          ...
          <el-button v-if="scope.row.downCount.one>0" size="mini" @click="showDownDis(scope.$index, scope.row)">查看下级分销商</el-button>
          <el-button size="mini" @click="showClient(scope.$index, scope.row)">查看客户</el-button>
