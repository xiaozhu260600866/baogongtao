            ...
            <div class="ptools pb0 clearfix">
              <div class="search-box f-left">
                <el-form :inline="true" :model="listQuery">
                  ...
                  <el-form-item>
                    <el-select v-model="listQuery.validity" clearable placeholder="有效期限" style="width:150px;">
                      <el-option v-for="item in validityArr" :key="item.value" :label="item.name" :value="item.value" />
                    </el-select>
                  </el-form-item>
                  <el-form-item>
              ...
              <div class="f-right">
                ...
                <router-link to="/company/create/"><el-button type="primary"><i class="el-icon-plus" /> 新增企业</el-button></router-link>
              </div>
            </div>
                ...
                <el-table-column label="企业名称">
                  <template slot-scope="scope">
                    <div class="list-col-img">
                      ...
                      <router-link :to="'/company/show?id='+scope.row.id" class="fc-blue"><div class="list-txt">{{ scope.row.name }}</div></router-link>
                    </div>
                  </template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <el-dropdown @command="handleCommand">
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item><router-link :to="'/company/show?id='+scope.row.id" class="fc-blue"><i class="el-icon-s-order" /> 详情</router-link></el-dropdown-item>
                        <el-dropdown-item><router-link :to="'/company/edit?id='+scope.row.id" class="fc-blue"><i class="el-icon-edit" /> 编辑</router-link></el-dropdown-item>
...
<script>
import { lists, del } from '@/api/company'
...
export default {
	components: { Pagination, CompanyStatus },
	data() {
		return {
			selectLists: [],
	...
	methods: {
		...
		changeStatus(row) {
			store({ id: row.user_id, status: row.account_status }).then(res => {
				// this.listsData = res.data.lists;
			})
			// console.log(row);
		},
		...
		handleCommand(command) {
			if (command.command == 'delete') {
				this.tytConfirm({ msg: '确认要删除该企业吗？' }, this).then(obj => {
