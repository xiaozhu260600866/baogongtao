          ...
          <div class="dx-main">
            <div class="list-box">
              <el-tabs v-model="tabActive" @tab-click="handleClick">
                <el-tab-pane label="资金账号" name="capital_account">
                  <capital-account />
                </el-tab-pane>
                <el-tab-pane v-for="(item, index) in tabs" :label="item.remark" :name="item.name">
                  <attribute-lists :type="item.id" />
                </el-tab-pane>
              </el-tabs>
...
import CapitalAccount from '@/views/finance/config/components/CapitalAccount'
export default {
	components: { AttributeLists, CapitalAccount },
	data() {
		return {
			tabs: [],
			tabActive: 'capital_account',
			type: ''
		}
	},
	mounted() {
	...
	methods: {
		...
		getData() {
			attributeTypes({ category: 2 }).then(res => {
