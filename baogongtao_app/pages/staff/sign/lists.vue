	...
	<view>
		...
		<view class="sign-lists">
			<view class="sign-item" v-for="item in listsData" @click="detail(item)">
				...
				<view class="task">有效期：{{ item.start_date }} 至 {{ item.end_date }}</view>
				<view class="time" v-if="item.status==1">签署时间：{{ item.sign_at }}</view>
			</view>
...
export default {
	...
	methods: {
		getListsData(reload=0){
			...
			this.listQuery.user_id = this.sysStaff.user_id;
		...
		detail(item){
			if(item.status==0) return this.linkTo('/pages/staff/sign/show?id=' + item.id,1);
		}
	},
