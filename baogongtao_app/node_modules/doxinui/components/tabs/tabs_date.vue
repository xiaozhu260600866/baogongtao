<template>
	<view class="dxi-date-nav">
		...
		<view class="dxi-nav-order" :class="{'show_date':showDate,'is-down':isDown}" :style="{top:isDown?'0':orderTop*2+'rpx',zIndex:zIndex}">
			...
			<view class="order-right" :class="{'btn-hide':btnHide}">
				<dx-button :type="btnType" size="medium" @click="search" block>{{btnTxt?btnTxt:'搜索'}}</dx-button>
			</view>
		</view>
	</view>
</template>
...
.dxi-nav-order{display: flex;align-items: center;padding: 24upx;background-color: #fff;border-top: 1upx #eee solid;transition: all 0.3s ease-in-out;transform: translate3d(0, -100%, 0);visibility: hidden;transform-origin: center;position: absolute;top: 100upx;width: 100%;left: 0;box-shadow: 0 4px 8px 0 hsla(0,6%,58%,.3);}
...
.dxi-nav-order .btn-hide{display: none;}

.dxi-nav-order.is-down{position: relative;box-shadow: inherit;transform: none;height: 0;padding: 0 24rpx;transition: none;}
.show_date.is-down{transform: none;height: 100%;padding: 24rpx;}
</style>