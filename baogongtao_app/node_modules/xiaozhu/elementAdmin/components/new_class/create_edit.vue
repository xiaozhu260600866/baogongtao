<template>
    <div>
        <el-dialog title="新建或编辑" :visible.sync="dialogVisible" width="40%" :before-close="handleClose" append-to-body>
            <el-form :model="ruleForm" ref="ruleForm" class="pt20" createEditLabel="100px">
				
                 <el-form-item v-if="parent && type == 'add'" label="父级名称" class="flex">
                      <el-input v-model="parent.name"  :disabled='true' ></el-input>
                </el-form-item>
                <el-form-item :label="v.label" :prop="v.prop" class="flex" :rules="v.type == 'password'  && ruleForm.id ? '' : getRule(v)"
                    v-for="v in fields" v-if="v.type && !v.levShow || v.levShow == lev">
                    <el-input v-model="ruleForm[v.prop]" v-if="v.type == 'text'" :disabled='v.disabled' :style="v.style ? v.style : '' "></el-input>
                    ...
                    <el-input v-model="ruleForm[v.prop]" v-if="v.type == 'textarea'" type="textarea" :disabled='v.disabled'
                        :autosize="{ minRows: 2, maxRows: 4}" :style="v.style ? v.style : '' "></el-input>
                    <el-select v-model="ruleForm[v.prop]" placeholder="请选择" v-if="v.type == 'select'" :disabled='v.disabled'
                        :style="v.style ? v.style : '' " :multiple="v.multiple ? v.multiple : false">
                        <el-option :label="q.label" :value="q.value" v-for="q in data[v.datakey]" v-if="data[v.datakey]"></el-option>
                        <el-option :label="q.label" :value="q.value" v-for="q in v.data" v-if="v.data"></el-option>
                    </el-select>
                    <div v-if="v.type == 'manyselect'">
                        <el-cascader placeholder="请输入要搜索的分类" expand-trigger="hover" :options="data[v.datakey]" v-model="ruleForm[v.prop]"
                            style="width:300px" :props="{multiple: v.multiple ? v.multiple : false}"></el-cascader>
                    </div>
				
                    <div v-if="v.type == 'location' && !v.levShow || v.type == 'location' && v.levShow == lev" class="flex">
                        <el-input v-model="ruleForm[v.prop]" class="CinputWidth" :disabled='v.disabled' :style="v.style ? v.style : '' "></el-input>
					...
					<template scope="scope" v-if="v['append_form_'+lev+'_'+v.prop]">
                ...
                if(row){
                    ...
                    this.fields.forEach(v => {
                        if (v.type == 'upload') {
                            ...
                            let uploadVal  = row[v.prop] ? this.splitCover(row[v.prop], v.upurl) : [];
