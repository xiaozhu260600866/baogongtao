<template>
	<div class="app-container">
		<el-row>
			<el-col :span="24">
				<el-col :span="listsClass.classAction ? 6 : 0" class="pr10" v-if="listsClass.classAction">
					<div class="dmp">
						<div class="dx-container">
							<div class="dx-main">
								<div class="list-box" style="padding: 0;">
									<new-class @clickCallBack="clickCallBack" :data="data[listsClass.prop]" :fields="listsClass.classData" :type="listsClass.classType" :form-action="listsClass.classAction" :class-children-hidden="false" :add-lev="listsClass.addLev" v-if="data[listsClass.prop]">
									</new-class>
								</div>
							</div>
						</div>
				...
				<el-col :span="listsClass.classAction ? 18 : 24">
					<div class="dx-container">
						<tyt-page-header :content="title"></tyt-page-header>


						<div class="dx-main">
							<!-- 如果有搜索start -->
							<div class="ptools pb0 clearfix" v-if="searchFields.length">
									...
									<el-form :inline="true" :model="data.query" class="flex">
										<div class="flex1">
											<el-form-item v-for="v in searchFields" :label="v.label">
												<el-input :placeholder="v.placeholder ? v.placeholder : '请输入'+v.label"
													v-model="data.query[v.prop]" v-if="!v.type || v.type == 'text'">
												</el-input>
												<el-select v-model="data.query[v.prop]" placeholder="请选择"
													v-else-if="v.type == 'select'">
													<el-option :label="q.label" :value="q.value"
														v-for="q in data[v.datakey]" v-if="v.datakey"></el-option>
													<el-option :label="q.label" :value="q.value" v-for="q in v.data"
														v-if="v.data"></el-option>
												</el-select>
												<div v-else-if="v.type == 'betweenDate'">
													...
													<el-date-picker type="date"
														...
														v-model="data.query[v.prop+'_end'] " value-format="yyyy-MM-dd">
													</el-date-picker>
												</div>
												<div v-else-if="v.type == 'manyselect'">
													<el-cascader placeholder="请输入要搜索的分类" expand-trigger="hover"
														:options="data[v.datakey]" v-model="data.query[v.prop]"
												...
												<el-button-group v-else-if="v.type == 'searchYear'">
													<el-button :type="data.query[v.prop] == year.prop ? 'primary' : '' "
											...
											<el-form-item>
												<el-button type="primary" click="search" v-if="searchFields"
													@click="search"><i class="el-icon-search"></i> 搜索</el-button>
												<el-button type="primary" @click="$refs.category.ajax()"
													v-if="classAction">
													类别管理</el-button>
												<el-button type="success" @click="exportExcel" v-if="exportUrl">导出
												</el-button>
												<el-button type="success" v-if="uploadData && uploadData.url"
								...
								<div class="f-right">
									<el-button type="primary" @click.native="$refs.createEdit.ajax('',data,fields)"
										v-if="canCreate">{{createLabel ? createLabel :'新建'}}</el-button>
									<slot name="headerR" />
								</div>
							</div>
							<!-- 如果有搜索end -->
							<!-- 如果有tarbarsStart -->
							<div class="list-box " v-if="tarbars">
								<el-tabs v-model="data.query[tarbars.prop]" @tab-click="$parent.ajax">
									<el-tab-pane :label="v.count ? v.label+ '('+data[v.datakey]+')':v.label"
										:name="''+v.value" v-for="v in tarbars.data"></el-tab-pane>
								</el-tabs>
							</div>
							<div>
								<slot name="tableTop" />
							...
							<!-- 如果有tarbarsEnd -->
							<div class="list-box">
								<el-table ref="multipleTable" :data="data.lists" border style="width: 100%"
									@selection-change="selsChange" v-loading="data.listLoading"
									@sort-change="sortChange">
									<el-table-column type="selection" align="center" width="50" v-if="checkAll"> </el-table-column>
										<el-table-column :prop="v.prop" :label="v.label" :width="v.width"
											:align="v.align ? v.align:'left'" :min-width="v.minWidth" v-if="!v.hidden"
											:sortable="v.sortable ? 'custom' : false" v-for="(v,key) in tableFields" v-show="v['hidden_'+v.prop]? canShow(v):true">
											<template scope="scope"
												v-if="v.type || !v.type && v['append_table_'+v.prop]">
												...
												<div v-if="v.type && v.type == 'sort'">
													<el-input v-model="scope.row[v.prop]"
														@change="editField(scope.row,v.prop,v.url)"></el-input>
												</div>
												<div v-if="v.type && v.type == 'select'">
													<el-select v-model="scope.row[v.prop]" placeholder="请选择"
														...
														<el-option :label="q.label" :value="q.value"
															v-for="q in data[v.datakey]" v-if="v.datakey"></el-option>
														<el-option :label="q.label" :value="q.value"
															v-for="q in scope.row.data" v-if="scope.row.data">
														</el-option>
													</el-select>

												</div>
												<slot :name="'append_table_'+v.prop" :row="scope.row"
													:$index='scope.$index'
													v-if="!v.type && v['append_table_'+v.prop]" />
											</template>
										</el-table-column>
									<el-table-column label="操作" :width="operateWidth ? operateWidth : '160'" v-if="!operateHide">
										<template scope="scope">
											...
											<div class="cell_group" v-else>
												...
												<el-button type="primary" size="mini"
													@click.native="$refs.createEdit.ajax(scope.row,data,fields,'info')"
													v-if="showInfo">查看</el-button>
												<el-button type="primary" size="mini"
													@click="changeStatus(scope.$index, scope.row,'1')"
													v-if="scope.row.status!=1 && auditAction">审核通过</el-button>
												<el-button type="primary" size="mini"
													@click="changeStatus(scope.$index, scope.row,'2')"
													v-if="scope.row.status!=2 && auditAction">审核拒绝</el-button>
												<div class="slot_btn">
													<slot name="operate" :row="scope.row"></slot>
												</div>
												<el-button class="canDel" size="mini"
													@click.native="handleDel(scope.$index,scope.row)" v-if="canDel">删除
												</el-button>
											</div>
										</template>
									</el-table-column>
								...
								<div class="list-bottom clearfix">
									<div class="f-left flex">
										<el-button type="danger" @click="delAll" :disabled="this.sels.length===0 "
											v-if="checkAll && canDel && !hiddenDelAll"><i
												class="el-icon-delete-solid"></i>
											批量删除</el-button>
										<el-button type="danger" @click="puawayAll(1)" :disabled="this.sels.length===0 "
											v-if="putaway">批量上架</el-button>
										<el-button type="danger" @click="puawayAll(0)" :disabled="this.sels.length===0 "
											v-if="putaway">批量下架</el-button>
										<slot name="tableLeft" :row="sels" />
									</div>
									<div class="f-right">
										<el-pagination @size-change="handleSizeChange"
											@current-change="handleCurrentChange" :current-page="data.thisPage"
											:page-sizes="[pageSize]" :page-size="pageSize"
		...
		<my-create-edit @createBeforeCallBack="createBeforeCallBack" @submitBeforeCallBack="submitBeforeCallBack"
			ref="createEdit" :fields="formFields" :data="data" :createAction="createAction" :editAction="editAction"
			:createEditLabel="createEditLabel" :createEditDiagWidth="createEditDiagWidth" :lengthDiag="lengthDiag">
		...
		<uploadFile :action="uploadUrl()" ref="upload" :downLoadUrl="uploadData.downloadUrl"
			v-if="uploadData && uploadData.url"></uploadFile>


		<new-class ref="category" :diag="true" :data="data.category" :fields="classFields" :addLev="classLev"
			:no="classno" v-if="classAction && classFields.length" :type="classType" :formAction="classAction"
			:classChildrenHidden="classChildrenHidden" :canupload="canupload">
			<div :slot="'append_class_table_'+ field.prop" slot-scope="scope" v-for="field in classFields">
				<slot :name="'append_class_table_'+ field.prop" :row="scope.row" />
			</div>
		</new-class>
		<new-class ref="category" :diag="true" :data="data.category" :addLev="classLev" :no="classno"
			v-if="classAction &&classFields.length == 0" :type="classType" :formAction="classAction"
			:classChildrenHidden="classChildrenHidden" :canupload="canupload">
	...
	export default {
		...
		mounted() {
			...
			if (this.$route.meta.params) {
				for (let key2 in this.$route.meta.params) {
		...
		methods: {
			...
			handleDel(index, item) {
				...
				let delAction = this.delAction ? this.delAction : '/ajax/mydel';
