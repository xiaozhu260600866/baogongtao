	...
	<div class="flex" style="display: flex;align-items: flex-start;">
		...
		<div class="flex1">
            <div v-if="uploadLength == 1">
                <el-upload v-if="!uploadPic" class="avatar-uploader" :data="uploadData" :action="getSiteName()+'/ajax/uploadpic?api_token='+getUser().api_token+'&source=admin'" :show-file-list="false"
            ...
            <div v-else>
                <el-upload  :file-list="fileList" :action="getSiteName()+'/ajax/uploadpic?api_token='+getUser().api_token+'&source=admin'" :data="uploadData"
		...
		methods: {
			...
			uploadSuccess(response, file, fileList) {
				...
				if(that.uploadLength == 1){
					...
					file.url = that.getSiteName() + '/upload/images/'+that.upurl + '/'+ file.response.filename;
