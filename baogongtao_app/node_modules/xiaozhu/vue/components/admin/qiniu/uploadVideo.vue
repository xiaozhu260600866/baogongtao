<template>
    <div>
        <div style="position: relative" v-if="newFile.length == 0">
            <el-progress :percentage="percentage" class="progress" status="success" v-show="percentShow"></el-progress>
            <el-button element-loading-text="拼命上传中" @click="uploadQiniu" type="primary" size="small">上传</el-button>
            <input type="file" ref="inputfile" @change="fileChangeHandle" style="width:50px;height:30px;display:block;top:0;opacity: 0;position:absolute" />
        </div>
        <div v-else style="position: relative">
            <div v-for="(v,key) in newFile" v-if="v.status == 1">
                <a href="javascript:;" @click="del(key,v.key)" class="el-icon-close" style="position: absolute;left: 0;z-index: 999;color:white;background-color: red;font-size: 20px;">
                </a>
    ...
    export default {
        ...
        methods: {
            ...
            del(key,filename){
                ...
                let start = filename.indexOf('com');
