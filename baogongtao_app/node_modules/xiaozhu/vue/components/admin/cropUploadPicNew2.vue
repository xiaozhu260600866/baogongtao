	...
	<section>
		<el-dialog title="选择图片" :visible.sync="dialogFormVisible" top="141px" :before-close="handleClose" width="800px" append-to-body>
			<div class="flex">
				<div id="cut_container" style=" width:400px;margin-right: 30px">
					...
					<div class="select">
						...
						<input type="file" :id="'change_btn_'+uploadId" style="width: 80%; margin-bottom: 15px;display: block;opacity: 0;height: 40px;position: relative;z-index: 2;" @change="fileChange($event)">
					</div>
			...
			<el-form-item class="picUpload mt20">
				...
				<el-button type="primary" @click="upload2()">发布<span v-if="percent"></span></el-button>
			</el-form-item>
		</el-dialog>
	</section>
</template>
	...
	methods: {
		ajax(item) {
			this.$nextTick(() => {
				...
				let file = $('#change_btn_' + this.uploadId)[0];
				if (file) {
					file.value = '';
				}
