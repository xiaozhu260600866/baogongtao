...
myfun.install = (Vue, options) => {
    ...
    Vue.prototype.makeMenu = (roleArr, leftMenu, type) => {
        let isAdmin = type == "isAdmin" ? Vue.prototype.getUser().role == 1 : false;
    ...
    Vue.prototype.splitCover = (cover, path) => {
        ...
        let fileList = [];
        let covers = cover.split(",");
        for (var i = 0; i < covers.length; i++) {
            let url = Vue.prototype.getSiteName() + '/upload/images/' + path + '/' + covers[i];
    ...
    Vue.prototype.splitQiniu = (cover) => {
        ...
        for (var i = 0; i < covers.length; i++) {
            ...
            let start = covers[i].indexOf('com');
            let filename = covers[i].substr(start+4,end-start-1)
    ...
    Vue.prototype.authJson = () => {
        ...
        let role_menu = Vue.prototype.getUser().role_menus;
        return {
