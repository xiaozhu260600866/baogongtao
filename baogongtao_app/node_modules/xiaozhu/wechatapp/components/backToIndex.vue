<!-- 调用 
<backToIndex ></backToIndex>
-->
<template>
	<div v-if="source == 'share' " @click="toIndex2">
		<floatBtn2 type="1" title="回首页" :myclass="myclass"></floatBtn2>
	</div>
</template>
<script>
import floatBtn2 from "./floatBtn";
export default {
	props: ['text', 'Bottom', 'url', 'urlType', "myclass"],
	data() {
		return {
			source: ''
		}
	},
	methods: {
		toIndex2() {
			console.log('123');
			if (this.getSiteName() == 'https://boss.doxinsoft.com') {
				let role_type = wx.getStorageSync('role_type');
				console.log(role_type);
				if (role_type == 0) {
					return this.goto("/pages/cardLists/main");
				}
				if (role_type == 1) {
					return this.goto("/pages/user/website/products/lists/main", 2);
				}
				if (role_type == 2) {

					return this.goto("/pages/shop/index/main", 2);
				}
				if (role_type == 3) {
					return this.goto("/pages/cardLists/main");
				}
				if (role_type == 4) {
					return this.goto("/pages/index/main",2);
				}
			} else {
				return this.goto("/pages/index/main", 2);
			}

		}
	},
	mounted() {
		setTimeout(() => {
			if (this.$parent.data.query.shareSource && this.$parent.data.show) {
				this.source = this.$parent.data.query.shareSource;
			} else {
				this.source = "";
			}
			console.log(this.source);
			console.log(123007);
		},50)

	},
	components: {
		floatBtn2
	}

}

</script>
