...
const store = new Vuex.Store({
	state: {
		version: "1.0.0",
		apiUrl: "https://www.baogongtao.com",
		apiInterfaceUrl: "https://www.baogongtao.com/api",
		appId: "dalshop",
		appSecret: "xDo0xu11hQIC9QwSi5x1Ccop90QILAbj",
		token: uni.getStorageSync("token"),
		isLogin: uni.getStorageSync("sysUser") ? true : false,
		...
		sysConfig:uni.getStorageSync("sysConfig"),
		examining: uni.getStorageSync("examining")
	},
	mutations: {
		login(state, payload) {
			//uni.setStorageSync("sysUser", payload.data);
			console.log(1111111)
		...
		logout(state) {
			...
			state.isLogin = false
			uni.removeStorageSync("sysStaff");
			uni.removeStorageSync("sysCompany");
		},
		...
		setCompanyInfo(state, payload) {
			console.log(payload)
			//uni.removeStorageSync("sysUser");
			uni.setStorageSync("sysCompany", payload.data);
		},
		// 设置员工信息
		setStaffInfo(state, payload) {
			console.log(payload)
			uni.removeStorageSync("sysUser");
			uni.setStorageSync("sysStaff", payload.data);
		},
		// 设置APP信息
		...
		// 设置是否正在审核
		setExamine(state, payload) {
			state.examining = payload;
			uni.setStorageSync("examining", payload);
		},
		// 设置系统信息
		setSysConfig(state, payload) {
			state.sysConfig = payload;
			uni.setStorageSync("sysConfig", payload);
		}
	},
	actions: {
		// 查看公共数据版本是否更新
