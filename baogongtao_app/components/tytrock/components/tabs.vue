<template>
	<view class="tui-tabs-view" :class="[isFixed?'tui-tabs-fixed':'tui-tabs-relative',unlined?'tui-unlined':'']" :style="{height:height+'rpx',padding:`0 ${padding}rpx`,background:bgColor,top:isFixed?top+'px':'auto'}">
		<view v-for="(item,index) in tabs" :key="index" class="tui-tabs-item" :style="{width:selfItemWidth}" @tap.stop="swichTabs(index)">
		...
		<view class="tui-tabs-slider" :style="{transform:'translateX('+scrollLeft+'rpx)',width:sliderWidth+'rpx',height:
	...
	export default {
		...
		data() {
			return {
				winWidth: 0,
				scrollLeft: 0,
				selfItemWidth: this.itemWidth,
			};
		},
		mounted(){
			this.selfItemWidth = (1/this.tabs.length)*100 + '%';
		...
		methods: {
			checkCor: function() {
				...
				*/
				...
				let width = 750 - padding * 2
				let left = (width / tabsNum - this.sliderWidth) / 2 + padding
				let scrollLeft = left
				if (this.currentTab > 0) {
					scrollLeft = scrollLeft + (width / tabsNum) * this.currentTab
				}
