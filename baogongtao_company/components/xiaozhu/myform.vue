...
<template>
	<section>
		<form @submit="formSubmit" report-submit='true' v-if="userInfo" class="wxform block w-b100">
			...
			bindGetUserInfo(e) {
				uni.getUserProfile({
				      ...
				      success: (res1) => {
						...
						res.openid = openid;
						let userinfo = wx.getStorageSync('wxUser');
						if(!userinfo){
							uni.setStorageSync("wxUser", {nickName:res.nickName,avatarUrl:res.avatarUrl,gender:res.gender});
							this.userInfo  = res;
						}else{
							userinfo.nickName = res.nickName;
							userinfo.avatarUrl = res.avatarUrl;
							userinfo.gender = res.gender;
							uni.setStorageSync("wxUser", userinfo);
