		...
		methods:{
					...
					success: function(e){
						...
						if(_self.serverUrl){
									...
									uni.uploadFile({
										...
										success: function(res){
											...
											if(res.statusCode === 200){
												...
												console.log('success to upload image: ' + resData.data.filePath)
												resolve(resData.data.filePath)
											}else{
							...
							Promise.all(promiseWorkList).then((result)=>{
								...
								_self.$emit('add', {
									currentImagesPath: result,
									currentImagesTemp: imagePathArr,
			...
			deleteImage: function(e){
				...
				if(this.serverUrlDeleteImage){
					...
					let imageArr = deletedImagePath.split('/');
					formData.file = imageArr[imageArr.length-1];
					uni.request({
